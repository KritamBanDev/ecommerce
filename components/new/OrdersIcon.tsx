...
const OrdersIcon: React.FC<OrdersIconProps> = ({ orderCount }) => {
  ...
  return (
    <Link
      href={"/orders"}
      className="group relative focus:outline-none focus:ring-2 focus:ring-fuchsia-500"
      aria-label="View your orders"
      title="View your orders"
    >
      <span ref={iconRef} className="inline-flex items-center justify-center align-middle">
        <ListOrdered className="w-6 h-6 group-hover:text-fuchsia-600 hoverEffect transition-colors duration-200 align-middle bg-white rounded-full p-1 shadow hover:shadow-fuchsia-200/60" style={{ verticalAlign: 'middle' }} />
      </span>
      {orderCount > 0 && (
